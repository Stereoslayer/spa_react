...
export const selectVisibleBeforePaging = createSelector(selectProductsState, (st) => {
    ...
    const fav = st.favoritesOnly ? visible.filter((p) => st.liked[p.id]) : visible;
    const q = st.query.trim().toLowerCase();
    return q ? fav.filter(p =>
        p.title.toLowerCase().includes(q) ||
        (p.description ?? "").toLowerCase().includes(q)
    ) : fav;
});

export const selectVisibleCount = createSelector(selectVisibleBeforePaging, (list) => list.length);

export const selectTotalForPaging = createSelector(
    selectProductsState,
    selectVisibleCount,
    (st, visibleCount) => {
        const hasFilters = st.favoritesOnly || st.query.trim().length > 0;
        return hasFilters ? visibleCount : st.total; // 🔧 гибрид
    }
);

export const selectList = createSelector(selectProductsState, selectVisibleBeforePaging, (st, list) => {
